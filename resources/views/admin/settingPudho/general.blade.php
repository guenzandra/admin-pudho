...
.gs-wrap{font-family:Arial,sans-serif;padding:24px 20px;max-width:960px;margin:0 auto}
.gs-wrap *{font-family:Arial,sans-serif}
:root{
  --g-red:#C0392B;--g-red-h:#a93226;--g-red-light:#FDECEA;
  --g-white:#fff;--g-gray-50:#F9F9F9;--g-gray-100:#F1F1F1;
  --g-gray-200:#E0E0E0;--g-gray-400:#9E9E9E;--g-gray-600:#555;--g-gray-800:#222;
  --g-green:#27AE60;--g-green-light:#EAFAF1;
  --g-blue:#2980B9;--g-blue-light:#EAF4FB;
  --g-amber:#E67E22;--g-amber-light:#FEF9EC;
  --g-radius:8px;--g-radius-sm:5px;--g-radius-lg:12px;
}

/* ── Page header ────────────────────────── */
.gs-page-header{margin-bottom:24px}
.gs-page-title{font-size:20px;font-weight:700;color:var(--g-gray-800);display:flex;align-items:center;gap:10px}
.gs-page-sub{font-size:13px;color:var(--g-gray-400);margin-top:4px}

/* ── Card ───────────────────────────────── */
.gs-card{background:var(--g-white);border:1px solid var(--g-gray-200);border-radius:var(--g-radius-lg);margin-bottom:18px;overflow:hidden;position:relative}
.gs-card-header{display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:1px solid var(--g-gray-100);flex-wrap:wrap;gap:8px}
.gs-card-title{font-size:14px;font-weight:700;color:var(--g-gray-800);display:flex;align-items:center;gap:8px}
.gs-card-sub{font-size:11px;color:var(--g-gray-400);margin-top:2px}
.gs-card-body{padding:22px}

/* ── Buttons ────────────────────────────── */
.gs-btn{display:inline-flex;align-items:center;gap:6px;padding:9px 16px;border-radius:var(--g-radius);font-family:Arial;font-size:13px;font-weight:600;cursor:pointer;border:1.5px solid transparent;transition:all .15s}
.gs-btn-red{background:var(--g-red);color:#fff;border-color:var(--g-red)}
.gs-btn-red:hover{background:var(--g-red-h)}
.gs-btn-outline{background:var(--g-white);border-color:var(--g-gray-200);color:var(--g-gray-600)}
.gs-btn-outline:hover{border-color:var(--g-red);color:var(--g-red)}
.gs-btn-ghost{background:none;border:none;cursor:pointer;padding:6px 8px;border-radius:var(--g-radius-sm);color:var(--g-gray-400);display:flex;align-items:center;transition:all .15s}
.gs-btn-ghost:hover{color:var(--g-red);background:var(--g-red-light)}
.gs-btn-sm{padding:6px 12px;font-size:12px}
.gs-btn:disabled{opacity:.5;cursor:not-allowed}

/* ── Field ──────────────────────────────── */
.gs-field{margin-bottom:18px}
.gs-field:last-of-type{margin-bottom:0}
.gs-field label{display:flex;align-items:center;gap:7px;font-size:12px;font-weight:700;color:var(--g-gray-600);margin-bottom:6px}
.gs-field label svg{color:var(--g-red);flex-shrink:0}
.gs-input{width:100%;padding:10px 13px;border:1.5px solid var(--g-gray-200);border-radius:var(--g-radius);font-family:Arial;font-size:13px;color:var(--g-gray-800);outline:none;background:var(--g-white);transition:border .2s}
.gs-input:focus{border-color:var(--g-red)}
.gs-input.error{border-color:var(--g-red);background:var(--g-red-light)}
textarea.gs-input{resize:vertical;min-height:80px;line-height:1.5}
.gs-select{width:100%;padding:10px 13px;border:1.5px solid var(--g-gray-200);border-radius:var(--g-radius);font-family:Arial;font-size:13px;color:var(--g-gray-800);outline:none;background:var(--g-white);cursor:pointer;transition:border .2s}
.gs-select:focus{border-color:var(--g-red)}
.gs-field-hint{font-size:11px;color:var(--g-gray-400);margin-top:5px}
.gs-field-err{font-size:11px;color:var(--g-red);margin-top:5px;display:none}
.gs-field-err.show{display:block}
.gs-char-count{font-size:11px;color:var(--g-gray-400);text-align:right;margin-top:4px}

/* ── File upload ────────────────────────── */
.gs-file-zone{border:2px dashed var(--g-gray-200);border-radius:var(--g-radius);padding:20px;text-align:center;cursor:pointer;transition:all .15s;background:var(--g-gray-50)}
.gs-file-zone:hover,.gs-file-zone.dragover{border-color:var(--g-red);background:var(--g-red-light)}
.gs-file-zone input{display:none}
.gs-file-zone-text{font-size:13px;color:var(--g-gray-400);margin-top:6px}
.gs-file-zone-text strong{color:var(--g-red)}
.gs-file-preview{display:flex;align-items:center;gap:12px;padding:10px 12px;background:var(--g-gray-50);border:1px solid var(--g-gray-200);border-radius:var(--g-radius);margin-top:10px}
.gs-file-preview img{width:40px;height:40px;object-fit:contain;border-radius:4px;border:1px solid var(--g-gray-200);background:#fff}
.gs-file-preview-name{font-size:12px;font-weight:700;color:var(--g-gray-800);flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.gs-file-preview-size{font-size:11px;color:var(--g-gray-400)}

/* ── Color picker row ───────────────────── */
.gs-color-row{display:flex;align-items:center;gap:10px}
.gs-color-input{width:44px;height:38px;padding:3px;border:1.5px solid var(--g-gray-200);border-radius:var(--g-radius);cursor:pointer;background:var(--g-white)}
.gs-color-input:focus{border-color:var(--g-red)}
.gs-color-hex{flex:1;max-width:120px}
.gs-color-swatch-row{display:flex;gap:6px;margin-top:8px;flex-wrap:wrap}
.gs-swatch{width:24px;height:24px;border-radius:50%;cursor:pointer;border:2px solid transparent;transition:all .15s;flex-shrink:0}
.gs-swatch:hover,.gs-swatch.active{border-color:var(--g-gray-800);transform:scale(1.15)}

/* ── Toggle ─────────────────────────────── */
.gs-toggle-row{display:flex;align-items:center;justify-content:space-between;padding:12px 0;border-bottom:1px solid var(--g-gray-100)}
.gs-toggle-row:last-child{border-bottom:none}
.gs-toggle-info{display:flex;align-items:center;gap:10px}
.gs-toggle-icon{width:34px;height:34px;border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0}
.gs-toggle-label{font-size:13px;font-weight:700;color:var(--g-gray-800)}
.gs-toggle-desc{font-size:11px;color:var(--g-gray-400);margin-top:2px}
.gs-toggle{position:relative;width:44px;height:24px;flex-shrink:0}
.gs-toggle input{opacity:0;width:0;height:0;position:absolute}
.gs-toggle-slider{position:absolute;inset:0;background:var(--g-gray-200);border-radius:24px;cursor:pointer;transition:background .2s}
.gs-toggle-slider::before{content:'';position:absolute;width:18px;height:18px;background:#fff;border-radius:50%;top:3px;left:3px;transition:transform .2s;box-shadow:0 1px 3px rgba(0,0,0,.2)}
.gs-toggle input:checked+.gs-toggle-slider{background:var(--g-red)}
.gs-toggle input:checked+.gs-toggle-slider::before{transform:translateX(20px)}

/* ── Font preview strip ─────────────────── */
.gs-font-preview{padding:12px 14px;background:var(--g-gray-50);border:1px solid var(--g-gray-200);border-radius:var(--g-radius);margin-top:8px;font-size:14px;color:var(--g-gray-800);transition:font-family .2s}

/* ── Theme preview bar ──────────────────── */
.gs-theme-preview{border-radius:var(--g-radius);overflow:hidden;border:1px solid var(--g-gray-200);margin-top:12px}
.gs-theme-preview-nav{height:36px;display:flex;align-items:center;padding:0 14px;gap:8px}
.gs-theme-preview-nav span{width:10px;height:10px;border-radius:50%}
.gs-theme-preview-body{padding:12px 14px;background:var(--g-white);font-size:12px;color:var(--g-gray-600)}
.gs-theme-preview-btn{display:inline-flex;align-items:center;gap:5px;padding:5px 12px;border-radius:20px;font-size:11px;font-weight:700;color:#fff;margin-top:6px}

/* ── Dark mode preview ──────────────────── */
.gs-dark-preview{border-radius:var(--g-radius);border:1px solid var(--g-gray-200);overflow:hidden;transition:all .3s;margin-top:12px}
.gs-dark-preview-bar{height:32px;background:#1e293b;display:flex;align-items:center;padding:0 12px;gap:6px}
.gs-dark-preview-bar span{font-size:11px;color:#94a3b8}
.gs-dark-preview-body{padding:12px 14px;background:#0f172a;color:#e2e8f0;font-size:12px}
.gs-dark-preview-body.light{background:var(--g-white);color:var(--g-gray-800)}

/* ── Section footer ─────────────────────── */
.gs-card-footer{padding:14px 22px;border-top:1px solid var(--g-gray-100);display:flex;justify-content:flex-end;gap:8px;background:var(--g-gray-50)}

/* ── Loading overlay ────────────────────── */
.gs-loading-overlay{display:none;position:absolute;inset:0;background:rgba(255,255,255,.82);z-index:10;align-items:center;justify-content:center;border-radius:var(--g-radius-lg)}
.gs-loading-overlay.show{display:flex}
@keyframes gs-spin{to{transform:rotate(360deg)}}
.gs-spinner{width:26px;height:26px;border:2.5px solid var(--g-red-light);border-top-color:var(--g-red);border-radius:50%;animation:gs-spin .7s linear infinite}

/* ── Btn loading ────────────────────────── */
.gs-btn-spinner{width:14px;height:14px;border:2px solid rgba(255,255,255,.4);border-top-color:#fff;border-radius:50%;animation:gs-spin .7s linear infinite;flex-shrink:0}

/* ── Overlay / Modal ────────────────────── */
.gs-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:9000;align-items:center;justify-content:center;padding:16px}
.gs-overlay.open{display:flex}
.gs-modal{background:var(--g-white);border-radius:var(--g-radius-lg);width:100%;max-width:460px;max-height:92vh;overflow-y:auto;box-shadow:0 8px 40px rgba(0,0,0,.18)}
.gs-modal-sm{max-width:360px}
.gs-modal-header{display:flex;align-items:center;justify-content:space-between;padding:16px 20px 13px;border-bottom:1px solid var(--g-gray-200);position:sticky;top:0;background:var(--g-white);z-index:1}
.gs-modal-title{font-size:15px;font-weight:700;color:var(--g-gray-800);display:flex;align-items:center;gap:8px}
.gs-modal-close{background:none;border:none;cursor:pointer;color:var(--g-gray-400);padding:4px;border-radius:var(--g-radius-sm);display:flex;transition:all .15s}
.gs-modal-close:hover{color:var(--g-red);background:var(--g-red-light)}
.gs-modal-body{padding:20px}
.gs-modal-footer{padding:13px 20px;border-top:1px solid var(--g-gray-200);display:flex;justify-content:flex-end;gap:8px;position:sticky;bottom:0;background:var(--g-white)}

/* ── Logo crop preview ──────────────────── */
.gs-logo-circle{width:80px;height:80px;border-radius:50%;border:3px solid var(--g-red);overflow:hidden;display:flex;align-items:center;justify-content:center;background:var(--g-gray-50);flex-shrink:0}
.gs-logo-circle img{width:100%;height:100%;object-fit:cover}

/* ── Toast ──────────────────────────────── */
.gs-toast-wrap{position:fixed;bottom:24px;right:24px;z-index:10000;display:flex;flex-direction:column;gap:8px;pointer-events:none}
.gs-toast{display:flex;align-items:center;gap:10px;color:#fff;padding:12px 16px;border-radius:var(--g-radius);font-size:13px;font-weight:600;box-shadow:0 4px 16px rgba(0,0,0,.2);opacity:0;transform:translateY(10px);transition:all .25s;pointer-events:auto;min-width:220px;max-width:340px}
.gs-toast.show{opacity:1;transform:translateY(0)}
.gs-toast.success{background:var(--g-green)}
.gs-toast.error{background:var(--g-red)}
.gs-toast.info{background:var(--g-blue)}
.gs-toast.warning{background:var(--g-amber)}
.gs-toast-msg{flex:1}
.gs-toast-x{background:none;border:none;cursor:pointer;color:rgba(255,255,255,.75);font-size:15px}
.gs-toast-x:hover{color:#fff}

/* ── Unsaved badge ──────────────────────── */
.gs-unsaved{display:none;font-size:11px;font-weight:700;color:var(--g-amber);background:var(--g-amber-light);padding:2px 8px;border-radius:20px;align-items:center;gap:4px}
.gs-unsaved.show{display:inline-flex}

/* ── Divider ────────────────────────────── */
.gs-divider{border:none;border-top:1px solid var(--g-gray-100);margin:16px 0}

/* ── Responsive ─────────────────────────── */
@media(max-width:640px){
  .gs-wrap{padding:14px 10px}
  .gs-modal{max-width:100%}
  .gs-color-row{flex-wrap:wrap}
  .gs-card-footer{flex-direction:column}
  .gs-card-footer .gs-btn{justify-content:center}
}
</style>
...
<div class="gs-wrap">
  ...
  <div class="gs-card">
    ...
    <div class="gs-card-body">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px" class="gs-field">
        <div>
          <label style="font-size:12px;font-weight:700;color:var(--g-gray-600);margin-bottom:6px;display:flex;align-items:center;gap:6px">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--g-red)" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
            Contact Email
          </label>
        ...
        <div>
          <label style="font-size:12px;font-weight:700;color:var(--g-gray-600);margin-bottom:6px;display:flex;align-items:center;gap:6px">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--g-red)" stroke-width="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12"/></svg>
            Phone
          </label>
      ...
      <div class="gs-field">
        <label>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M14.83 14.83a4 4 0 1 1 0-5.66"/></svg>
          Copyright Text
        </label>
  ...
  <div class="gs-card">
    ...
    <div class="gs-card-body">
      ...
      <div class="gs-field">
        <label>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2a10 10 0 0 1 10 10c0 5.52-4.48 10-10 10a10 10 0 0 1 0-20z"/></svg>
  ...
  <div class="gs-card">
    ...
    <div class="gs-card-body">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px" class="gs-field">
        <div>
          <label style="font-size:12px;font-weight:700;color:var(--g-gray-600);margin-bottom:6px;display:flex;align-items:center;gap:6px">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--g-red)" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/></svg>
            Background Color
          </label>
          <div class="gs-color-row">
        ...
        <div>
          <label style="font-size:12px;font-weight:700;color:var(--g-gray-600);margin-bottom:6px;display:flex;align-items:center;gap:6px">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--g-red)" stroke-width="2"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
            Text Color
          </label>
          <div class="gs-color-row">
        ...
        <div class="gs-toggle-row" style="padding:12px 14px">
          <div class="gs-toggle-info">
            <div class="gs-toggle-icon" style="background:var(--g-gray-100)">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--g-gray-600)" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
            </div>
            <div>
        ...
        <div class="gs-toggle-row" style="padding:12px 14px">
          <div class="gs-toggle-info">
            <div class="gs-toggle-icon" style="background:var(--g-blue-light)">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--g-blue)" stroke-width="2"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
            </div>
            <div>
        ...
        <div class="gs-toggle-row" style="padding:12px 14px">
          <div class="gs-toggle-info">
            <div class="gs-toggle-icon" style="background:var(--g-green-light)">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--g-green)" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
            </div>
            <div>
        ...
        <div class="gs-toggle-row" style="padding:12px 14px">
          <div class="gs-toggle-info">
            <div class="gs-toggle-icon" style="background:var(--g-amber-light)">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--g-amber)" stroke-width="2"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>
            </div>
            <div>
      ...
      <div style="margin-top:14px">
        ...
        <div class="gs-dark-preview" id="gsDarkPreviewWrap" style="display:none">
          <div class="gs-dark-preview-bar">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
    ...
    <div class="gs-card-body">
      <div style="border:1px solid var(--g-gray-200);border-radius:var(--g-radius);overflow:hidden">
        <div class="gs-toggle-row" style="padding:12px 14px">
          <div class="gs-toggle-info">
            <div class="gs-toggle-icon" style="background:var(--g-red-light)">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--g-red)" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
            </div>
            <div>
        ...
        <div class="gs-toggle-row" style="padding:12px 14px">
          <div class="gs-toggle-info">
            <div class="gs-toggle-icon" style="background:var(--g-blue-light)">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--g-blue)" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
            </div>
            <div>
        ...
        <div class="gs-toggle-row" style="padding:12px 14px">
          <div class="gs-toggle-info">
            <div class="gs-toggle-icon" style="background:var(--g-amber-light)">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--g-amber)" stroke-width="2"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg>
            </div>
            <div>
        ...
        <div class="gs-toggle-row" style="padding:12px 14px">
          <div class="gs-toggle-info">
            <div class="gs-toggle-icon" style="background:var(--g-green-light)">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--g-green)" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            </div>
            <div>
  ...
  <div class="gs-modal gs-modal-sm" role="dialog" aria-modal="true">
    ...
    <div class="gs-modal-footer" style="justify-content:center;gap:10px">
      ...
      <button class="gs-btn gs-btn-red" id="gsResetConfirmBtn">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-3.51"/></svg>
        Yes, Reset
      </button>
    </div>
  </div>
...
/* ═══════════════════════════════════════
   MODAL HELPERS
═══════════════════════════════════════ */
function gsOpenModal(id){ document.getElementById(id).classList.add('open'); document.body.style.overflow='hidden'; }
function gsCloseModal(id){ document.getElementById(id).classList.remove('open'); if(!document.querySelector('.gs-overlay.open')) document.body.style.overflow=''; }
document.querySelectorAll('.gs-overlay').forEach(o=>{ o.addEventListener('click',function(e){ if(e.target===this) gsCloseModal(this.id); }); });
document.addEventListener('keydown',e=>{ if(e.key==='Escape'){ const o=document.querySelector('.gs-overlay.open'); if(o) gsCloseModal(o.id); } });
...
function gsToast(msg, type='info'){
  ...
  const icons={success:'<polyline points="20 6 9 17 4 12"/>',error:'<line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>',warning:'<path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/>',info:'<circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>'};
...
/* ═══════════════════════════════════════
   LOADING & BTN STATE
═══════════════════════════════════════ */
function gsShowLoading(id){ document.getElementById(id).classList.add('show'); }
function gsHideLoading(id){ document.getElementById(id).classList.remove('show'); }
function gsBtnLoad(btn, on){
  if(on){ btn._o=btn.innerHTML; btn.disabled=true; btn.innerHTML=`<div class="gs-btn-spinner"></div> Saving…`; }
  else { btn.innerHTML=btn._o; btn.disabled=false; }
}

/* ═══════════════════════════════════════
   DIRTY / UNSAVED
═══════════════════════════════════════ */
function gsDirty(id){ document.getElementById(id).classList.add('show'); }
function gsClean(id){ document.getElementById(id).classList.remove('show'); }
...
function gsPreviewFile(input, previewId, zoneId){
  ...
  if(f.size>2*1024*1024){ gsToast('File exceeds 2 MB limit','error'); input.value=''; return; }
...
function gsHandleDrop(e, inputId, previewId, zoneId){
  ...
  gsPreviewFile(inp, previewId, zoneId); gsDirty('gsGenUnsaved');
}

/* ═══════════════════════════════════════
   CHAR COUNT
═══════════════════════════════════════ */
function gsCountChars(el, countId){ document.getElementById(countId).textContent=el.value.length; }
...
function gsPickSwatch(hex, colorId, hexId){
  ...
  document.querySelectorAll('.gs-color-swatch-row .gs-swatch').forEach(s=>{ s.classList.toggle('active', s.style.background===hex||s.style.backgroundColor===hex); });
  gsUpdateThemePreview(); gsDirty('gsThemeUnsaved');
}
...
function gsSaveGeneral(){
  ...
  if(!name){ document.getElementById('gsWebNameErr').classList.add('show'); gsToast('Website name is required','error'); return; }
  document.getElementById('gsWebNameErr').classList.remove('show');
  const btn=document.getElementById('gsSaveGenBtn');
  gsShowLoading('gsGenLoading'); gsBtnLoad(btn,true);
  /* Wire to backend:
  ...
  */
  setTimeout(()=>{ gsHideLoading('gsGenLoading'); gsBtnLoad(btn,false); gsClean('gsGenUnsaved'); gsToast('General settings saved successfully','success'); },1200);
}
...
function gsSaveTheme(){
  ...
  gsShowLoading('gsThemeLoading'); gsBtnLoad(btn,true);
  setTimeout(()=>{ gsHideLoading('gsThemeLoading'); gsBtnLoad(btn,false); gsClean('gsThemeUnsaved'); gsToast('Theme settings saved','success'); },1100);
}
...
function gsSaveAccount(){
  ...
  gsShowLoading('gsAccLoading'); gsBtnLoad(btn,true);
  setTimeout(()=>{ gsHideLoading('gsAccLoading'); gsBtnLoad(btn,false); gsClean('gsAccUnsaved'); gsToast('Account theme saved','success'); },1100);
}

/* ═══════════════════════════════════════
   SAVE ADVANCED
═══════════════════════════════════════ */
function gsSaveAdvanced(){ gsToast('Advanced settings saved','success'); }

/* ═══════════════════════════════════════
   RESET HANDLERS
═══════════════════════════════════════ */
let gsResetTarget=null;
function gsResetGeneral(){ gsResetTarget='general'; document.getElementById('gsResetConfirmBtn').onclick=gsDoReset; gsOpenModal('gsResetModal'); }
function gsResetTheme(){ gsResetTarget='theme'; document.getElementById('gsResetConfirmBtn').onclick=gsDoReset; gsOpenModal('gsResetModal'); }
function gsResetAccount(){ gsResetTarget='account'; document.getElementById('gsResetConfirmBtn').onclick=gsDoReset; gsOpenModal('gsResetModal'); }
function gsDoReset(){
  ...
  if(gsResetTarget==='general'){
    ...
    gsUpdateFontPreview(); gsUpdateThemePreview();
    ...
    gsUpdateAccPreview(); gsToggleDarkPreview();
...
function gsToggleMaintenance(cb){
  if(cb.checked){ cb.checked=false; gsOpenModal('gsMaintenanceModal'); }
  else { gsToast('Maintenance mode disabled','info'); }
}
function gsCancelMaintenance(){ gsCloseModal('gsMaintenanceModal'); }
