...
<div id="dms-root">
  <style>
    ...
    *::after {
    ...
    #dms-root .sb-brand-icon svg {
      width: 17px;
      height: 17px;
      color: #fff;
    }

    #dms-root .sb-brand-text {
      font-size: 14px;
      font-weight: 800;
      color: var(--ink);
      letter-spacing: -.03em;
    ...
    @media(max-width:640px) {
      ...
      #dms-root .doc-table td:nth-child(6),
      #dms-root .doc-table th:nth-child(6) {
        display: none
      }
    }
  </style>
  ...
  <div class="app-body">
    ...
    <div class="sidebar">
      ...
      <div class="sb-brand">
        ...
        <div>
          <div class="sb-brand-text">Laguna DMS</div>
          <div class="sb-brand-sub">Province of Laguna</div>
        </div>
      </div>
    ...
    <div class="main">
      ...
      <!-- FILTERS (search integrated here) -->
      <div class="filters-bar">
        <div class="filters-left">
          <div class="f-search">
            ...
            <input type="text" id="dmsLocalSearch" placeholder="Search documents…" oninput="dmsApplyFilter()">
          </div>
  ...
  <div class="help-body" id="dms-help-body">
    <div id="htab-guide">
      <div class="guide-item">
        ...
        <div class="guide-text">
          ...
          <p>Use the sidebar or tab row to switch between District I–IV.</p>
        </div>
      </div>
      <div class="guide-item">
        ...
        <div class="guide-text">
          ...
          <p>Click Upload Document. Fill in the title, district, municipality, and category, then attach a file.</p>
        </div>
      </div>
      <div class="guide-item">
        ...
        <div class="guide-text">
          ...
          <p>Click Create Category or Manage Categories from the sidebar.</p>
        </div>
      </div>
      <div class="guide-item">
        ...
        <div class="guide-text">
          <h4>Search & Filter</h4>
          <p>Use the search bar in the filter row to find documents. Narrow by type, category, municipality, or sort order.</p>
        </div>
      </div>
      <div class="guide-item">
        ...
        <div class="guide-text">
          ...
          <p>Hover any row to reveal View, Download, Edit, and Delete actions.</p>
        </div>
      </div>
    </div>
    <div id="htab-faq" style="display:none">
      ...
      <div class="faq-item" onclick="dmsFaqToggle(this)">
        ...
        <div class="faq-a">Yes, open Manage Categories and click the × on any category to remove it.</div>
      </div>
      <div class="faq-item" onclick="dmsFaqToggle(this)">
    ...
    <div id="htab-shortcuts" style="display:none">
      <div class="sc-row">Search documents<div class="kbd-g"><kbd>Ctrl</kbd><kbd>K</kbd></div>
      </div>
...
<!-- ══════════════════════════════════════════
     GLOBAL STYLES — modals live outside #dms-root
     ══════════════════════════════════════════ -->
<style>
  #dms-help-btn {
    position: fixed;
...
<div class="dms-modal-bg" id="dmsCategoryModal" onclick="dmsBgClose(event,'dmsCategoryModal')">
  <div class="dms-modal dms-modal-sm">
    ...
    <div class="modal-footer">
      <button class="dms-btn dms-btn-primary" onclick="dmsCloseModal('dmsCategoryModal')">Done</button>
    </div>
  </div>
</div>
...
<script>
  ...
  // Uploader pool — in production this comes from auth/user list
  const UPLOADERS = [{
      name: 'Maria Santos',
  ...
  // Logged-in admin — in production pull from your auth system
  const CURRENT_USER = UPLOADERS[3]; // Ramon Flores (IT Admin) as example
  ...
  function dmsGetFiltered(districtFilter) {
    const q = (document.getElementById('dmsLocalSearch')?.value || '').toLowerCase().trim();
    ...
    if (q) docs = docs.filter(d => d.name.toLowerCase().includes(q) || d.slug.toLowerCase().includes(q) || d.muni.toLowerCase().includes(q) || (d.uploader?.name || '').toLowerCase().includes(q));
    if (tf !== 'all') docs = docs.filter(d => d.type === tf);
    if (cf !== 'all') docs = docs.filter(d => d.cat === cf);
    if (mf !== 'all') docs = docs.filter(d => d.muni === mf);
    const srt = sf === 'date' ? (a, b) => new Date(b.date) - new Date(a.date) :
      sf === 'size' ? (a, b) => b.size - a.size :
  ...
  function dmsTableHTML(docs) {
    ...
    docs.forEach((d, i) => {
      ...
      const up = d.uploader || CURRENT_USER;
      const initials = up.name.split(' ').map(w => w[0]).join('').substring(0, 2).toUpperCase();
      rows += `<tr style="animation-delay:${delay}ms">
        ...
        <td>
          <div class="td-uploader">
            <div class="td-avatar" style="background:${dmsEsc(up.color)}">${initials}</div>
            <div>
              <div class="td-uploader-name">${dmsEsc(up.name)}</div>
              <div class="td-uploader-role">${dmsEsc(up.role)}</div>
            </div>
          </div>
        </td>
  ...
  function dmsUpdateMuniFilter() {
    ...
    const munis = ACTIVE_DISTRICT === 'all' ?
      [...new Set(DOCS.map(d => d.muni))].sort() :
  ...
  function dmsSubmitUpload() {
    ...
    setTimeout(() => {
      ...
      DOCS.push({
        ...
        uploader: CURRENT_USER // always stamp with current logged-in user
      });
  ...
  document.addEventListener('keydown', e => {
    ...
    if (c && e.key === 'k') {
      e.preventDefault();
      document.getElementById('dmsLocalSearch').focus()
    }
    if (c && e.key === 'u') {
      e.preventDefault();
      dmsOpenUpload()
